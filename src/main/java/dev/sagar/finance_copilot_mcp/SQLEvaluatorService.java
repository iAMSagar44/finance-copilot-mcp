...
class SQLEvaluatorService {
	...
	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(SQLEvaluatorService.class);
	...
	public SQLEvaluatorService(ChatClient.Builder builder, DatabaseService databaseService) {
		this.chatClient = builder.defaultOptions(OpenAiChatOptions.builder().model("o3-mini").temperature(1D).build())
			.build();
	...
	EvaluationResponse evaluateQuery(String userQuestion, String sqlQuery) throws IOException {
		...
		var evaluationResponse = chatClient.prompt()
			...
			.user(userSpec -> userSpec.text(USER_MESSAGE_TEMPLATE)
				...
				.param("sql_query", sqlQuery)
				.param("current_date", LocalDate.now().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.LONG)))
				.param("database_type", databaseType))
			.call()
			.entity(EvaluationResponse.class);

		logger.info("Evaluation: {} \n Feedback: {}", evaluationResponse.evaluation(), evaluationResponse.feedback());
